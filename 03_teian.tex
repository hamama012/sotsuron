...
仮想空間に置かれるユーザのアバターは手部分のみである。HTC Viveのコントローラにはコントローラの3DCGモデルが標準で用意されているが、現実感の付与を目的とした本研究ではコントローラよりも人間の手のモデルを使用した方が自然である。そのためMayaを用いて手の3DCGを作成し、Unityへエクスポートした。コントローラモデルの代わりに使用する。コライダーはアセットストアから入手したSABoneColiderを使用して可能な限り精密なものを作成した。
