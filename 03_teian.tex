...
SteamVR		& SteamVRのプレハブ\\
\hline 
\end{tabular}}
\end{table}

Petはペットアバターである。詳細は口述するペット管理部で述べる。
CameraRigはSteamVRプラグイン内にあるCameraRigプレハブのインスタンスであり、これをメインカメラとして用いる。プレハブとはGameObjectのコンポーネントとプロパティ全てを格納するテンプレートで、インスタンスを複数作成することが可能である。
SteamVRも同様に、SteamVRプラグイン内にあるSteamVRプレハブのインスタンスである。トラッキングスペースはデフォルトでTracking Universe Standing（ルームスケール）になっているが、本研究では着席状態で使用するため、Tracking Universe Seated（着席モード）に指定しCameraRigをプレイヤーの頭が来るであろう位置に配置した。
CameraRigの子オブジェクトとしてControllerがあり、本研究では右手で持つコントローラを示すController(right)のみを利用する。Controller及びその子オブジェクトであるhandを使用してユーザの動作を制御する。詳細は後述するユーザ管理部で述べる。

BackGroundにはライトを除いた環境を構成するオブジェクトを全て格納している。
木や草といったオブジェクトはUnityのアセットストアで入手したNature Starter Kit 2を使用して配置した
...
仮想空間に置かれるユーザのアバターは手部分のみである。HTC Viveのコントローラにはコントローラの3DCGモデルが標準で用意されているが、現実感の付与を目的とした本研究ではコントローラよりも人間の手のモデルを使用した方が自然である。そのためMayaを用いて手の3DCGを作成し、Unityへエクスポートした。コントローラモデルの代わりに使用する。
...
本研究では先行研究のバーチャルペットは使用せず、新しく柴犬を参考にMayaを用いて3DCGモデルを作成、Unityへエクスポートした。待機、歩く、走る、尻尾を振るの４種類のアニメーションが可能である。
